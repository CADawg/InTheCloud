...
let interestPoints = {
    ...
    'port2': [5, 8, "Port 2"],
    'port3': [12, 1, "Port 3"],
    'oilrig1': [2, 6, "Taco Alpha"],
    'oilrig2': [3, 2, "Taco Bravo"],
    'oilrig3': [6, 4, "Taco Charlie"],
    'oilrig4': [9, 7, "Nemo Echo"],
    'oilrig5': [11, 3, "Nemo Delta"],
}
...
function setSidebarStats(data) {
    let {total, _, trip} = data;

    $("#total-emissions-count").text(parseFloat(total).toFixed(3));
...
function startTrip() {
    ...
    for (let i = 0; i < placesWeMustGo.length - 1; i++) {
        setTimeout(() => {
            ...
            if (placesWeMustGo[i].includes("Port")) {
                cargo = 10;
            }

            currentTrip++;

            sendShipFromTo(placesWeMustGo[i], placesWeMustGo[i + 1]);
            setShipCargo(cargo);
            setSidebarStats({
...
function sendShipFromTo(start, end, sidebarStats) {
    ...
    if (direction === "left") {
        ...
        console.log("flip ship");
        $("#ship-container").css("transform", "scaleX(-1)");
    } else {
        $("#ship-container").css("transform", "scaleX(1)");
    }
